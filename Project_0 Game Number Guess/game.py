...
def think_number(up_border: int = 100) -> int: 
    ...
    if type(up_border) is int: # Проверим на корректность ввод границы
...
def mean_score(up_border:int = 100, prt_text: bool = True) -> int:
    """Функция считает число попыток 1000 раз и возвращает среднее число попыток.
    ...
    for i in range(999): # Делаем все 1000 попыток
        count_ls.append(number_guess(think_number(up_border),up_border, False))
    
    score = int(np.mean(count_ls)) # Считаем результаты
    
    if prt_text: # Проверка нужно ли печатать результаты
        print(f'Среднее число попыток для даипозона от 1 до {up_border} - {score}.')
